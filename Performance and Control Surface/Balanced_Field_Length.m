...
% The outputs are: (SI UNITS)
% BFL is the Balanced Field Length in Metres

% Doing some unit covnersions
W_to_lbs = W_to*2.20462;                          % Converting mass from kg to pounds
...
gamma_2 = arcsin((T_oei_lbf - D2_lbf)/W_to_lbs);   % Again another constant

G = gamma_2 - gamma_2min;                          % Another constant

T_bar = (0.75 * T_takeoff_static_lbf) * ((5+BPR) / (4+BPR)); % Another bloody constant

BFL_feet = (0.863/(1+(2.3*G))) * (((W_to_lbs/S_ft)/(rho*g*Cl_climb)) + h_obs) * ((1/((T_bar/W_to_lbs)-U)) + 2.7) + (655/sqrt_sigma);
