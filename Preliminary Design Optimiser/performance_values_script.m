...
for i = 1 : length(h)
    ...
    for j = 1 : length(h)
        ...
        CL_Cruise = (W_ini_1) / (0.5 * rho * (V_Cruise)^2 * Sref);
        CD_Total = CD_0_Total(2) + (K * (CL_Cruise)^2);
        D = 0.5 * rho * Sref * velocity^2 * CD_Total;
        Ps(i,j) = (velocity / W0) * (Thrust - D); % Finding out specific excess power
    end
    
    vstall(i) = sqrt((2 * W_ini_1 / Sref) / (rho * CL_max_clean)) / a;   % Stall speed to be plotted in Mach
    M_max(i) = (2 * T_dummy) / (rho * Sref * CD_Total * a^2); 
end
...
plot(vstall, h, '-r')
plot(M_max, h, '-b')
legend('Specific Excess Energy', 'Stall Boundary', 'Max velocity')
