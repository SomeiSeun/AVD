...
Sub ReadExternalFile(IA As Integer)
  ...
  Do
    ...
    'ikawa 01/24/03
    DTemp = libGL(IA) * libPcntOnMainGears(IA, ACN_mode) / 100 / _
    libNMainGears(IA) / libNTires(IA) / libCP(IA)  'ik033 uncommented by GFH 01/21/03
...
Sub WriteExternalFile()
  ...
  Dim FileName$, EFNo As Integer, I As Integer
...
'  Or ((libIndex <> new_libIndex) And (ACN_mode_true)) Then
  ...
  Else
    libPcntOnMainGears(IAC, ACN_mode) = libPcntOnMainGears(libIndex, ACN_mode)
...
'  TireContactArea = libTireContactArea(IAC) 'ik033
  ...
  RigidCutoff = StandardRigidCutoff
